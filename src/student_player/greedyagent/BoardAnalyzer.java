package student_player.greedyagent;
...
//		System.out.println("Printing final perspective");
	...
	private PentagoMove ExtractMove() {
		...
		while (iterations > 0) {
			...
			PentagoMove tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 1, 3,
					pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
			tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 1, 2, pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
			tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 1, 0, pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
			tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 0, 2, pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
			tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 0, 3, pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
			tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 2, 3, pbs.getTurnPlayer());
			if (MoveNotSuicide(tobechecked)) {
				return tobechecked;
			}
	...
	private PentagoMove GetPerfectSwap(PentagoBoardState copy, int[] flatcoords) {
		...
		Integer score=0;
		PentagoMove tobechecked = new PentagoMove(flatcoords[0], flatcoords[1], 1, 3,pbs.getTurnPlayer());
		if (MoveNotSuicide(tobechecked)) {
			copy=(PentagoBoardState) pbs.clone();
	...
	private boolean MoveNotSuicide(PentagoMove tobechecked) {
		...
		for (PentagoMove trial : temp.getAllLegalMoves()) {
			...
			if (temp2.getWinner() == 1 - temp2.getTurnPlayer()) {
				return false;
			}
		}
		return true;
	...
	private Move canIWin() {
		...
		for (PentagoMove trial : pbs.getAllLegalMoves()) {
			...
			if (temp.getWinner() == 1 - temp.getTurnPlayer()) {
				return trial;
			}
		}
